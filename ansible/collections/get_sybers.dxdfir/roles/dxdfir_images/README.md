...
per image via `dxdfir_images_build_overrides` args), so every Linux-capable EZ tool ships as an
...
via dxdfir/plaso: esedb/srum (libesedb) and prefetch (libscca)): no shell, no python, uid 2000
by this role.
...
- **no shell and no python** except where the tool needs them: `dxdfir/yara`
  keeps `sh` (its scan loop is a shell script), `dxdfir/volatility` and
  `dxdfir/plaso` keep python (the tools are python); `dxdfir/zeek`,
  `dxdfir/suricata`, `dxdfir/evtxecmd` carry neither
...
`cid=$(docker create dxdfir/<tool>:latest); docker export "$cid" | tar -t | grep -E 'apt-get|dpkg|sudo|/pip|/sh$|python3'; docker rm -f "$cid"`.
...
**Kept only where the tool needs it**: `dxdfir/yara` keeps `sh` (its per-file
scan loop is a shell script — the image ENTRYPOINT); `dxdfir/volatility` and
`dxdfir/plaso` keep `python3` (the tools *are* python). `dxdfir/zeek`,
`dxdfir/suricata` and `dxdfir/evtxecmd` carry **no shell and no python** at all.
...
|---|---|---|
| `dxdfir_images_context` | `<repo>/docker` | Build context (holds `<name>/Dockerfile` + `hardening/harden.yml`). |

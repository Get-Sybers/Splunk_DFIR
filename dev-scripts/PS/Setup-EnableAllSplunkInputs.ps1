# Setup-EnableAllSplunkInputs.ps1
# PowerShell version of setup-enable-all-splunk-inputs.sh
...
try {
    ...
    $replacements = ([regex]::Matches($newContent, "disabled = false")).Count
    
    Write-Host "Enabled $replacements inputs in $INPUTS_CONF_PATH" -ForegroundColor Green
    Write-Host "All Splunk inputs have been enabled successfully." -ForegroundColor Green
}
catch {

...
Deploy + deliver roles: **`dfir_ingest_sofelk`** (deliver processed output into a
watch dir — the `<type>/…` tree the Elastic-native stack's Filebeat and the retiring
SOF-ELK both read), **`dfir_deploy_sofelk`** (builds the from-source SOF-ELK stack —
...
(`python/get_sybers_dfir/detect/rules/`, ES|QL/EQL loaded and validated by
`get_sybers_dfir.detect.rules_loader`) run by Elastic's Detection Engine on the
...
these roles for you:
```bash
dxdfir process zeek --pipeline elastic  # = the dfir_zeek role, preflight → process → verify
...
```bash
ansible-playbook playbooks/dfir-process-zeek.yml -e dfir_zeek_pipeline=elastic
...
| Variables, no hardcoding | Everything flows through `defaults/main.yml` with the `dfir_<role>_` prefix; inputs validated with `assert` at play start; the elastic\|sofelk decision is a resolved variable (`dfir_<role>_out_dir`), not duplicated task files. |
...
| Cross-platform conditionals | **Principle implemented, mechanics inapplicable:** the practice's point — one adaptable unit instead of near-identical copies — is exactly the shared `process.yml` + resolved `dfir_<role>_out_dir`, applied to the elastic\|sofelk axis. OS-family `when` ladders have no surface: targets are tool containers on a Linux analysis host, single-platform by design. |

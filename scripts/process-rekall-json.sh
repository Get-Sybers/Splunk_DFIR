...
for MEMORY_FILE in "${ALL_FILES[@]}"; do
    ...
    if [[ $successful_files -eq 0 ]]; then
        echo "WARNING: All Rekall plugins failed for $FILENAME - check logs for details" | tee -a "$HOST_OUTPUT_DIR/logs/$FILENAME.log"
    else
        echo "SUCCESS: Processed $FILENAME with $successful_files successful extractions" | tee -a "$HOST_OUTPUT_DIR/logs/$FILENAME.log"
    fi

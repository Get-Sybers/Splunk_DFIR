...
def get_validation_file_location():
    return os.path.join(os.path.dirname(os.path.abspath(__file__)), py_file)


def exec_anaconda(model_content=None):
    ...
    try:
        ...
        try:
            logger.info("Found correct PSC version, executing model validation")
            my_process = get_validation_file_location()
            # create subprocess be sure to pass stdin, stdout, stderr
            p = subprocess.Popen(
                [python_path, my_process, "-i", model_content],
            ...
            if "Error" in str(stdoutdata):
                logger.debug(f"Error during model file validation: {str(stdoutdata)}")
                return str(stdoutdata)

        except subprocess.CalledProcessError as e:
            raise RuntimeError(
                f'Error encountered while validating file using Python for Scientific Computing: {e}'
            )
...
def exec_anaconda_or_die(model_content):
    try:
        resp = exec_anaconda(model_content)
        return resp
    except Exception as e:
        raise RuntimeError(e)
        import cexc

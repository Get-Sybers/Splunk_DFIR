...
#   ./tests/run-molecule.sh dfir_zeek ...    run specific roles only
#
...
#   MOLECULE_SAMPLE_EVTX      .evtx log            (dfir_evtx)
#   MOLECULE_EVTXECMD_DIR     EvtxECmd release dir (dfir_evtx)
#   MOLECULE_SAMPLE_IMAGE     raw/E01 disk image   (dfir_plaso)
#   MOLECULE_SAMPLE_MEMORY    memory image         (dfir_volatility)
#
...
ROLES_DIR="$REPO_ROOT/ansible/collections/get_sybers.dfir/roles"
IMAGE="${MOLECULE_IMAGE:-dfir/molecule:latest}"

# Roles whose scenarios validate real behaviour. dfir_velociraptor's scenario
# is layout-only (no engine) and is excluded from the default set — pass it
# explicitly to run it anyway.
DEFAULT_ROLES=(dfir_signatures dfir_zeek dfir_evtx dfir_plaso dfir_volatility)
ROLES=("${@:-${DEFAULT_ROLES[@]}}")
